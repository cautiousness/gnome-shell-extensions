...
function initSoup()
{
  ...
  soupSession.timeout = 6;
}

function initDirectSoup()
{
  directSoupSession = new Soup.Session();
  directSoupSession.user_agent = USER_AGENT;
  directSoupSession.timeout = 6;
  directSoupSession.proxy_resolver = Gio.SimpleProxyResolver.new(null, []);
}
...
export function freeSoup()
{
  ...
  soupSession = null;
  if(directSoupSession) directSoupSession.abort();
  directSoupSession = null;
}
...
export function getDirectSoupSession()
{
  if(directSoupSession === null) initDirectSoup();
  return directSoupSession;
}
