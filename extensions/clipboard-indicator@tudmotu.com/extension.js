...
function dedupeEntries(entries) {
    ...
    for (const entry of entries) {
        const duplicateIndex = result.findIndex(item => item.equals(entry));

        if (duplicateIndex >= 0) {
            const existing = result[duplicateIndex];
            entry.favorite = entry.isFavorite() || existing.isFavorite();
...
const ClipboardIndicator = GObject.registerClass({
    ...
    async _buildMenu () {
        let that = this;
        const clipHistory = dedupeEntries(await this._getCache());
    ...
    #addToCache (entry) {
        const entries = dedupeEntries(this.clipItemsRadioGroup
